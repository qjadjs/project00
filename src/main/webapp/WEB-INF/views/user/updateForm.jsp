...
<legend class=c1>회원정보수정</legend>
	<form class=d1 action="/user/update" method="post" onsubmit="return checked()">
		...
		비밀번호 <input type="text" name="userPw" class="pw" id="password" value="${user.userPw}" onfocus="this.value=''" ><br>
		비밀번호 확인<input type="password" name="passwordConfrim" class="pw"
					id="passwordConfirm" placeholder="비밀번호  확인"><br>
		이름 <input type="text" name="userName" class="name" id="userName" value="${user.userName} " onfocus="this.value=''"><br>
		이메일 <input type="text" name="userEmail" class="email" id="userEmail" value="${user.userEmail}" onfocus="this.value=''"><br>
		<div class=btn-div>
